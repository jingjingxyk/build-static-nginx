...
echo 	@"$(LINK)" /nologo $(LDFLAGS) $(LDFLAGS_CLI) $(LIBS) $(CLI_GLOBAL_OBJS_RESP) $(PHP_GLOBAL_OBJS_RESP) $(STATIC_EXT_OBJS_RESP) $(STATIC_EXT_LIBS) $(ASM_OBJS)  $(BUILD_DIR)\php.exe.res /out:$(BUILD_DIR)\php.exe  >> %X_MAKEFILE%
